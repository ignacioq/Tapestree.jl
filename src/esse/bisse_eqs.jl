...
function fbisse(du::Array{Float64,1}, 
  ...
  @inbounds begin
    ...
    du[1]= -1.0 * (p1 + p3 + p5) * u[1] + p5 * u[2] + 2.0 * p1 * u[3] * u[1]
    du[2]= -1.0 * (p2 + p4 + p6) * u[2] + p6 * u[1] + 2.0 * p2 * u[4] * u[2]
    du[3]= -1.0 * (p1 + p3 + p5) * u[3] + p3 + p5 * u[4] + p1 * u[3]^2
    du[4]= -1.0 * (p2 + p4 + p6) * u[4] + p4 + p6 * u[3] + p2 * u[4]^2
  end
