...
function llik_gbm_ss(tree::iTbd,
  ...
  if istip(tree)
    ...
    ll1, dλ1, ssλ1, ssμ1, nλ1, ns, ne =
      llik_gbm_ss(tree.d1, α, σλ, σμ, δt, srδt, ns, ne)
    ll2, dλ2, ssλ2, ssμ2, nλ2, ns, ne =
...
function ll_gbm_b_ss(lλv ::Array{Float64,1},
  ...
  @inbounds begin
    ...
    ll = llλ = llμ = llbdλ = llbdμ = ssλ = ssμ = nλ = 0.0
    if nI > 0
      @turbo for i in Base.OneTo(nI)
        lλvi   = lλv[i]
        lμvi   = lμv[i]
    ...
    if fdt > 0.0
      ...
      nλ   += 1.0
      ll   -= (fdt*(exp(0.5*(lλvi + lλvi1))) + fdt*(exp(0.5*(lμvi + lμvi1))))
    end
...
function llr_gbm_b_sep(lλp ::Array{Float64,1},
  ...
  @inbounds begin
    ...
    llrbm = llrbmλ = llrbmμ = llrbdλ = llrbdμ = ssrλ = ssrμ = 0.0
    if nI > 0
      @turbo for i in Base.OneTo(nI)
        lλpi    = lλp[i]
        lλci    = lλc[i]
...
function _ss_dd_b(lλv::Array{Float64,1},
  ...
  return (lλv[nI+2] - lλv[1]), ssλ, ssμ, n
end
