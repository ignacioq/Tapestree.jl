...
function insane_gbmbd(tree    ::sT_label,
  ...
  mc = m_surv_gbmbd(th, log(λc), log(μc), αi, σλi, σμi, δt, srδt, 15_000, crown)
...
function update_α!(αc     ::Float64,
  ...
  mp  = m_surv_gbmbd(th, λ0, μ0, αp, σλ, σμ, δt, srδt, 15_000, crown)
  llr = log(mp/mc)
...
function update_σ!(σλc     ::Float64,
  ...
  mp  = m_surv_gbmbd(th, λ0, μ0, α, σλp, σμp, δt, srδt, 15_000, crown)
  llr = log(mp/mc)
