...
"""
function insane_cfbd_gp(tree    ::sTfbd, 
                        out_file::String;
                        λprior  ::Float64,
                        μprior  ::Float64,
                        niter   ::Int64,
  ...
  llc, prc, tree, λc, μc, λtn, μtn, idf, dabr = 
      mcmc_burn_cfbd(tree, n, th, tune_int, λprior, μprior, 
        nburn, ϵi, λi, μi, λtni, μtni, scalef, idf, wbf, dabr, pup, pupdp, 
        prints, svf)

  # mcmc
  R, tree = mcmc_cfbd(tree, llc, prc, λc, μc, λprior, μprior,
...
function mcmc_burn_cfbd(tree    ::sTfbd,
  ...
  llc = llik_cfbd(tree, λc, μc) - svf(λc, μc, th)
...
function graftp(tree::sTfbd,
  ...
  t0, t0h = sim_cfbd_b(λc, μc, th, 100)
  ...
  if t0h < th
    ...
    # likelihood ratio
    llr = llik_cfbd(t0, λc, μc) + log(2.0*λc) 
...
function prunep(tree::sTfbd,
  ...
  if ng > 0
    ...
    # likelihood ratio
    llr = - stree_ll_cfbd(tree, 0.0, λc, μc, dri, ldr, wpr, 0, 1) - 
            log(2.0 * λc)
