...
function insane_gbmct(tree    ::sT_label,
  ...
  ϵc = ϵi
  mc = m_surv_gbmct(th, log(λc), αi, σλi, ϵc, δt, srδt, 115_000, stem)
...
function update_fs!(bix    ::Int64,
  ...
  # if terminal
  if it(bi)
    ξp, llr = fsbi_t_ct(bi, lλ(ξc)[1], α, σλ, ϵ, δt, srδt)
    drλ  = 0.0
    ssrλ = 0.0
    Σrλ  = 0.0
  # if internal
    ...
    ξp, llr, drλ, ssrλ, Σrλ =
      fsbi_i(bi, ξc, Ξ[d1(bi)], Ξ[d2(bi)], lλ(ξc)[1], α, σλ, ϵ, δt, srδt)
  end
...
function fsbi_t_ct(bi  ::iBffs,
  ...
  t0, na, nn, llr =
    _sim_gbmct_t(e(bi), λ0, α, σλ, ϵ, δt, srδt, lc, lU, Iρi, 0, 1, 1_000)
...
function update_α_ϵ!(αc     ::Float64,
  ...
  mp  = m_surv_gbmct(th, λ0, αp, σλ, ϵ, δt, srδt, 15_000, stem)
  llr = log(mp/mc)
...
function update_σ_ϵ!(σλc     ::Float64,
  ...
  mp  = m_surv_gbmct(th, λ0, α, σλp, ϵ, δt, srδt, 15_000, stem)
  llr = log(mp/mc)
...
function update_ϵ!(ϵc   ::Float64,
  ...
  mp  = m_surv_gbmct(th, λ0, α, σλ, ϵp, δt, srδt, 15_000, stem)
  ϵr  = log(ϵp/ϵc)
