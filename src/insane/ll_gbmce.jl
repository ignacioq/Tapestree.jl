...
#   if ix === ldr
#     llb, lli, lls = llis_gbm_f(tree, α, σλ, μ, δt, srδt)
#     return llb, lli, lls
#   elseif ix < ldr
#     ifx1 = isfix(tree.d1)
#     if ifx1 && isfix(tree.d2)
#       ix += 1
#       if dri[ix]
#         llb, lli, lls = br_llis(tree.d1, α, σλ, μ, δt, srδt, dri, ldr, ix)
#       else
#         llb, lli, lls = br_llis(tree.d2, α, σλ, μ, δt, srδt, dri, ldr, ix)
#       end
#     elseif ifx1
#       llb, lli, lls = br_llis(tree.d1, α, σλ, μ, δt, srδt, dri, ldr, ix)
#     else
...
# Estimate `gbmce` likelihood for the tree in a fix branch.
# """
# function llis_gbm_f(tree::iTgbmce,
#                     α   ::Float64,
#                     σλ  ::Float64,
#                     μ   ::Float64,
#                     δt  ::Float64,
...
#   if istip(tree)
#     llb, lli, lls = 
#       llis_gbm_b(lλ(tree), α, σλ, μ, δt, fdt(tree), srδt, false, false)
#   else
#     llb, lli, lls = 
#       llis_gbm_b(lλ(tree), α, σλ, μ, δt, fdt(tree), srδt, true, false)

#     ifx1 = isfix(tree.d1)
#     if ifx1 && isfix(tree.d2)
#       return llb, lli, lls
#     elseif ifx1
#       llb1, lli1, lls1 = llis_gbm_f(tree.d1, α, σλ, μ, δt, srδt)
#       llb2, lli2, lls2 = llis_gbm(  tree.d2, α, σλ, μ, δt, srδt)
#       llb += llb1 + llb2
#       lli += lli1 + lli2
#       lls += lls1 + lls2
#     else
#       llb1, lli1, lls1 = llis_gbm(  tree.d1, α, σλ, μ, δt, srδt)
#       llb2, lli2, lls2 = llis_gbm_f(tree.d2, α, σλ, μ, δt, srδt)
#       llb += llb1 + llb2
#       lli += lli1 + lli2
#       lls += lls1 + lls2
#     end
...
# Returns the inference and simulated log-likelihood for a `iTgbmce` 
# according to `gbmce`.
# """
# function llis_gbm(tree::iTgbmce, 
#                    α   ::Float64,
#                    σλ  ::Float64, 
#                    μ   ::Float64,
#                    δt  ::Float64,
...
#   if istip(tree)
#     llb, lli, lls = 
#       llis_gbm_b(lλ(tree), α, σλ, μ, δt, fdt(tree), srδt, 
...
# """
#     llis_gbm_b(lλv ::Array{Float64,1},
#                α   ::Float64,
#                σλ  ::Float64,
#                μ   ::Float64,
#                δt  ::Float64, 
...
# Returns the simulated and the inferred log-likelihood for a branch 
# according to `gbmce`.
# """
# @inline function llis_gbm_b(lλv ::Array{Float64,1},
#                              α   ::Float64,
#                              σλ  ::Float64,
#                              μ   ::Float64,
#                              δt  ::Float64, 
...
#     lli = 0.0  # birth-death likelihood for inference
#     lls = 0.0  # birth-death likelihood for simulation
#     llb = 0.0  # gbm likelihood
#     lλvi = lλv[1]
#     @simd for i in Base.OneTo(nI)
#       lλvi1 = lλv[i+1]
#       llb  += (lλvi1 - lλvi - α*δt)^2
#       lli  += exp(0.5*(lλvi + lλvi1))
#       lls  += log1p(-(δt*(exp(0.5*(lλvi + lλvi1)) + μ)))
#       lλvi  = lλvi1
#     end
#     llb = llb*(-0.5/((σλ*srδt)^2)) - Float64(nI)*(log(σλ*srδt) + 0.5*log(2.0π))
#     lli += Float64(nI)*μ
#     lli *= -δt
...
#     # add final non-standard `δt`
#     if fdt > 0.0
#       llb += ldnorm_bm(lλvi1, lλvi + α*fdt, sqrt(fdt)*σλ)
#       lli -= fdt*(exp(0.5*(lλvi + lλvi1)) + μ)
#       # if speciation
#       if λev
#         lli += lλvi1
#         lls += 0.5*(lλvi1 + lλvi)
...
#         lls += log(μ)
#       else
#         lls += log1p(-(fdt*(exp(0.5*(lλvi + lλvi1)) + μ)))
#       end
#     elseif λev
#       lli += lλvi1
#       lls += lλvi1
