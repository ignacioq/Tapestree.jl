...
Returns the log-likelihood for a `iTce` according to `gbm-ce`.
"""
function llik_gbm_sep(Ξ   ::Vector{iTce},
                      idf ::Vector{iBffs},
                      α   ::Float64,
                      σλ  ::Float64,
                      μ   ::Float64,
  ...
  @inbounds begin
    ...
    llbd = 0.0
    for i in Base.OneTo(lastindex(Ξ))
      llbm, llbd = (llbm, llbd) .+ llik_gbm_sep(Ξ[i], α, σλ, μ, δt, srδt)
      if d2(idf[i]) > 0
        llbd += λt(Ξ[i])
      end
    end
...
Returns the log-likelihood for a `iTce` according to `gbmce`.
"""
function llik_gbm_sep(tree::iTce,
                      α   ::Float64,
                      σλ  ::Float64,
                      μ   ::Float64,
                      δt  ::Float64,
                      srδt::Float64)
  if istip(tree)
    ll_gbm_b_sep(lλ(tree), α, σλ, μ, δt, fdt(tree), srδt, false, isextinct(tree))
  else
    ll_gbm_b_sep(lλ(tree), α, σλ, μ, δt, fdt(tree), srδt, true, false) .+
    llik_gbm_sep(tree.d1, α, σλ, μ, δt, srδt)                          .+
    llik_gbm_sep(tree.d2, α, σλ, μ, δt, srδt)
  end
end
...
Returns the log-likelihood for a branch according to `gbmce`.
"""
@inline function ll_gbm_b_sep(lλv ::Array{Float64,1},
                              α   ::Float64,
                              σλ  ::Float64,
                              μ   ::Float64,
                              δt  ::Float64,
  ...
  if fdt > 0.0
    lλvi  = lλv[nI+1]
    llbm  += ldnorm_bm(lλvi1, lλvi + α*fdt, sqrt(fdt)*σλ)
    llbd -= fdt*(exp(0.5*(lλvi + lλvi1)) + μ)
  end
