    ...
    while true
      ...
      if t <= δt
        ...
        if divev(λm, μm, t)
          ...
          if λorμ(λm, μm)
            nsp += 1
            na  += 2
            return iTgbmbd(iTgbmbd(0.0, δt, 0.0, false, false,
                             Float64[λt1, λt1], Float64[μt1, μt1]),
                           iTgbmbd(0.0, δt, 0.0, false, false,
                             Float64[λt1, λt1], Float64[μt1, μt1]),
                           bt, δt, t, false, false, λv, μv), na, nsp
          # if extinction
          else
            return iTgbmbd(bt, δt, t, true, false, λv, μv), na, nsp
          end
        end

        na += 1
        return iTgbmbd(bt, δt, t, false, false, λv, μv), na, nsp
      end
      ...
      if divev(λm, μm, δt)
        ...
        if λorμ(λm, μm)
          nsp += 1
          td1, na, nsp =
            _sim_gbmbd(t, λt1, μt1, α, σλ, σμ, δt, srδt, na, nsp, nlim)
          td2, na, nsp =
            _sim_gbmbd(t, λt1, μt1, α, σλ, σμ, δt, srδt, na, nsp, nlim)

          return iTgbmbd(td1, td2, bt, δt, δt, false, false, λv, μv), na, nsp
        # if extinction
        else
          return iTgbmbd(bt, δt, δt, true, false, λv, μv), na, nsp
        end
      end
...
function _sim_gbmbd(nsδt::Float64,
  ...
  if divev(λm, μm, nsδt)
    ...
    if λorμ(λm, μm)
      nsp += 1
      td1, na, nsp = _sim_gbmbd(t, λt1, μt1, α, σλ, σμ, δt, srδt, na, nsp, nlim)
      td2, na, nsp = _sim_gbmbd(t, λt1, μt1, α, σλ, σμ, δt, srδt, na, nsp, nlim)

      return iTgbmbd(td1, td2, bt, δt, nsδt, false, false, λv, μv), na, nsp
    else
    # if extinction
      return iTgbmbd(bt, δt, nsδt, true, false, λv, μv), na, nsp
    end
  end
    ...
    while true
      ...
      if t <= δt
        ...
        if divev(λm, μm, t)
          ...
          # if extinction
            ...
            return iTgbmbd(bt, δt, t, true, false, λv, μv), surv, nsp
          end
        end

        return iTgbmbd(bt, δt, t, false, false, λv, μv), true, nsp
      end
      ...
      if divev(λm, μm, δt)
        ...
        if λorμ(λm, μm)
          ...
          return iTgbmbd(td1, td2, bt, δt, δt, false, false, λv, μv), surv, nsp
        # if extinction
        else
          return iTgbmbd(bt, δt, δt, true, false, λv, μv), surv, nsp
        end
      end
