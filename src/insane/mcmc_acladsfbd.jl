...
"""
    insane_acladsfbd(tree    ::sTf_label;
                     ...
                     nburn   ::Int64                 = 200,
                     nflush  ::Int64                 = nthin,
                     ofile   ::String                = string(homedir(), "/cladsfbad"),
                     λi      ::Float64               = NaN,
                     μi      ::Float64               = NaN,
                     ϵi      ::Float64               = 0.2,
                     ψi      ::Float64               = NaN,
...
"""
function insane_acladsfbd(tree    ::sTf_label;
                          λ0_prior::NTuple{2,Float64}     = (0.05, 148.41),
                          μ0_prior::NTuple{2,Float64}     = (0.05, 148.41),
                          αλ_prior::NTuple{2,Float64}     = (0.0, 1.0),
                          αμ_prior::NTuple{2,Float64}     = (0.0, 1.0),
  ...
  Ξ = make_Ξ(idf, λc, μc, acTfbd)

  # survival
  mc = m_surv_acladsfbd(th, log(λc), log(μc), αλi, αμi, σλi, σμi, 1_000, surv)
  ...
  @info "running asymmetrical cladogenetic fossilised birth-death"

  # burn-in phase
  Ξ, idf, llc, prc, αλc, αμc, σλc, σμc, mc, ns, ne, nf, 
  L, ddλ, ddμ, ssλ, ssμ, stnλ, stnμ =
    mcmc_buarn_cladsfbd(Ξ, idf, 
  ...
  # mcmc
  r, treev = 
    mcmc_claadsfbd(Ξ, idf, llc, prc, αλc, αμc, σλc, σμc, ψc, mc, th, rmλ, surv, 
...
"""
    mcmc_buarn_cladsfbd(Ξ       ::Vector{acTfbd},
                       idf     ::Vector{iBffs},
...
"""
function mcmc_buarn_cladsfbd(Ξ       ::Vector{acTfbd},
                            idf     ::Vector{iBffs},
                            λ0_prior::NTuple{2,Float64},
                            μ0_prior::NTuple{2,Float64},
                            αλ_prior::NTuple{2,Float64},
      ...
      if pupi === 1
        ...
        llc, prc, αλc, mc = 
          update_αλ!(αλc, lλ(Ξ[1]), lμ(Ξ[1]), αμc, σλc, σμc, 
            (ns + rmλ), ddλ, llc, prc, mc, th, surv, αλ_prior)

        # update ssλ with new drift `αλ`
        ssλ = _ss(Ξ, idf, αλc, lλ)
      ...
      elseif pupi === 2
        ...
        llc, prc, αμc, mc = 
          update_αμ!(αμc, lλ(Ξ[1]), lμ(Ξ[1]), αλc, σλc, σμc, 
            (ns + rmλ), ddμ, llc, prc, mc, th, surv, αμ_prior)

       # update ssμ with new drift `αμ`
        ssμ = _ss(Ξ, idf, αμc, lμ)
      ...
      elseif pupi === 3
        ...
        llc, prc, σλc, σμc, mc =
          update_σ!(σλc, σμc, lλ(Ξ[1])[1], lμ(Ξ[1])[1], αλc, αμc, ssλ, ssμ, 
            (ns + rmλ), llc, prc, mc, th, surv, σλ_prior, σμ_prior)
...
"""
    mcmc_claadsfbd(Ξ       ::Vector{acTfbd},
                  idf     ::Vector{iBffs},
                  llc     ::Float64,
                  prc     ::Float64,
                  αλc     ::Float64,
...
"""
function mcmc_claadsfbd(Ξ       ::Vector{acTfbd},
                       idf     ::Vector{iBffs},
                       llc     ::Float64,
                       prc     ::Float64,
                       αλc     ::Float64,
  ...
  treev = acTfbd[]          # make tree log vector
  io    = IOBuffer()       # buffer 
...
function update_αλ!(αλc     ::Float64,
  ...
  mp  = m_surv_acladsfbd(th, λ0, μ0, αλp, αμ, σλ, σμ, 1_000, surv)
  llr = log(mp/mc)
...
function update_αμ!(αμc     ::Float64,
  ...
  mp  = m_surv_acladsfbd(th, λ0, μ0, αλ, αμp, σλ, σμ, 1_000, surv)
  llr = log(mp/mc)
...
function update_σ!(σλc     ::Float64,
  ...
  mp  = m_surv_acladsfbd(th, λ0, μ0, αλ, αμ, σλp, σμp, 1_000, surv)
  llr = log(mp/mc)
...
"""
function update_scale!(Ξ       ::Vector{acTfbd},
                       idf     ::Vector{iBffs},
                       αλ      ::Float64,
                       αμ      ::Float64,
                       σλ      ::Float64,
  ...
  if lU < llr + prr + log(1000.0/mc)
    ...
    # survival
    mp   = m_surv_acladsfbd(th, lλ0 + s, lμ0, αλ, αμ, σλ, σμ, 1_000, surv)
    llr += log(mp/mc)
  ...
  if lU < llr + prr + log(1000.0/mc)
    ...
    # add survival ratio
    mp   = m_surv_acladsfbd(th, lλ0, lμ0 + s, αλ, αμ, σλ, σμ, 1_000, surv)
    llr += log(mp/mc)
...
function update_fs!(bix ::Int64,
  ...
  if iszero(d1(bi))
    ...
    if isfossil(bi)
      ...
      if !isfinite(llr)
         ξp, llr = 
          fsbi_et(acTfbd_wofe(ξc), bi, αλ, αμ, σλ, σμ, ψ, ψts, ixf)
      end
  ...
  if isfinite(llr)
    ...
    llc, ddλ, ddμ, ssλ, ssμ, ns, ne = 
      llik_claadsfbd_track!(ξc, αλ, αμ, σλ, σμ, ψ, tii, ψts, ixi, nep, 
        llc, L, ddλ, ddμ, ssλ, ssμ, ns, ne, -)
    llc, ddλ, ddμ, ssλ, ssμ, ns, ne = 
      llik_claadsfbd_track!(ξp, αλ, αμ, σλ, σμ, ψ, tii, ψts, ixi, nep, 
        llc, L, ddλ, ddμ, ssλ, ssμ, ns, ne, +)
...
function fsbi_t(bi ::iBffs,
  ...
  nep = lastindex(ψts) + 1
  t0, na, nn, llr =
    _sim_claadsfbd_t(e(bi), λi, μi, αλ, αμ, σλ, σμ, ψ, ψts, 
...
function fsbi_t(bi ::iBffs,
  ...
  nep = lastindex(ψts) + 1
  t0, na, af, nn =
    _sim_claadsfbd_i(ti(bi), tf(bi), λi, μi, αλ, αμ, σλ, σμ, ψ, ψts, 
...
function fsbi_m(bi ::iBffs,
  ...
  empty!(μfs)
  nep = lastindex(ψts) + 1
  t0, na, af, nn = _sim_claadsfbd_i(ti(bi), tf(bi), λi, μi, αλ, αμ, σλ, σμ, ψ, 
 ...
 if lU < lr && na < 500
    ...
    if istip(tree)
      if !isfix(tree) && isalive(tree)
        ...
        # simulate
        stree, na, nn, lr =
          _sim_claadsfbd_it(t, lλ(tree), lμ(tree), αλ, αμ, σλ, σμ, ψ, ψts, 
...
"""
function fossiltip_sim!(tree::acTfbd,
                        t   ::Float64,
                        αλ  ::Float64,
                        αμ  ::Float64,
                        σλ  ::Float64,
