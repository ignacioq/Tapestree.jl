...
function upnode!(λ      ::Array{Float64,1},
                 ω1     ::Float64,
                 ω0     ::Float64,
                 avg_Δx ::Array{Array{Float64,1},1},
                 triad  ::Array{Int64,1},
                 Y      ::Array{Int64,3},
    ...
    for j = Base.OneTo(narea)
      ...
      if pr < nedge
        ...
        bit_rejsam!(Y, bridx_a[j][pr], brs[pr,2,j], 
                    λj1, ω1, λj2, ω0, avg_Δx[j][pr], brδt[pr])
      end

      # for daughter branch 1
      bit_rejsam!(Y, bridx_a[j][d1], brs[d1,2,j], 
                  λj1, ω1, λj2, ω0, avg_Δx[j][d1], brδt[d1])

      # for daughter branch 2
      bit_rejsam!(Y, bridx_a[j][d2], brs[d2,2,j], 
                  λj1, ω1, λj2, ω0, avg_Δx[j][d2], brδt[d2])
...
function samplenode!(λ     ::Array{Float64,1},
  ...
  @inbounds begin
    ...
    for j = Base.OneTo(narea)
      ...
      # transition probabilities for the trio
      ppr_1, ppr_2 = 
        Ptrfast_start(λ[1], ω1, λ[2], ω0, avg_Δx[j][pr], brl_pr, brs[pr,1,j])
      pd1_1, pd1_2 = 
        Ptrfast_end(  λ[1], ω1, λ[2], ω0, avg_Δx[j][d1], brl_d1, brs[d1,2,j])
      pd2_1, pd2_2 = 
        Ptrfast_end(  λ[1], ω1, λ[2], ω0, avg_Δx[j][d2], brl_d2, brs[d2,2,j])
