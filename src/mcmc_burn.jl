...
function burn_tribe(total_llf     ::Function,
  ...
  const lapi = fill(NaN, ntip)             # lineage average for ωx > 0
  const lani = fill(NaN, ntip)             # lineage average for ωx < 0
  ...
  total_llf(Xc, Yc, LApc, LDc, ωxc, ω1c, ω0c, λ1p, λ0c, stemevc, brs, σ²c) -
  total_llf(Xc, Yc, LApc, LDc, ωxc, ω1c, ω0c, λ1c, λ0c, stemevc, brs, σ²c)
